...
<body class="p-4">
    <button onclick="scrollToBottom()" class="fixed bottom-0 right-0 m-4 p-2 bg-black opacity-80 text-white rounded-full">
          ...
          <tbody>
              ...
              <tr>
                ...
                <td colspan="3" class="border border-black">
                    @if($mustahik->hubungan == 'orang_tua')
                        Orang Tua
                    @elseif($mustahik->hubungan == 'mertua')
                        Mertua
                    @elseif($mustahik->hubungan == 'suami')
                        Suami
                    @elseif($mustahik->hubungan == 'istri')
                        Istri
                    @elseif($mustahik->hubungan == 'anak')
                        Anak
                    @endif
                </td>
            </tr>
...
<section id="tanda_tangan" class="my-4 mx-4">
  ...
  <div class="flex flex-col">
    <button type="button" id="tanda_tangan" class="px-4 py-6 w-full text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 mt-12" onclick="openModal()">Validasi dan Tanda Tangani</button>
    @if($mustahik->signed_koordinator)
    <a href="{{ route('form-print', $mustahik->id) }}" class="px-4 py-6 w-full text-sm font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 mt-2 text-center ">Print</a>
    @endif
  </div>
  ...
  <!-- Modal -->
  <form  id="signatureModal" action="{{ route('form-update-validasi', $mustahik->id) }}" method="POST" class="fixed z-10 inset-0 overflow-y-auto hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    @csrf
    @method('PUT')
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          ...
          <div class="mt-2">
            <div id="sig_koordinator" class="signature border border-gray-300 w-full rounded-lg p-2" style="height: 200px;"></div>
  ...
  <script>
    ...
    function submitSignature() {
      var data = $("#sig_koordinator").jSignature("getData", "svgbase64");
      $("#signature64_koordinator").val(data);
      closeModal();

...
@section('container')
    ...
    <form method="POST" action="{{ route('user-update', $user->id) }}" class="pt-20 flex flex-col space-y-4 max-w-screen-xl mx-auto">
        @csrf
        @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div class="">
                        ...
                        <div class="flex flex-col gap-2">
                            @if (old('level', $user->level) == 2)
                            <label for="admin" class="inline-flex items-center cursor-pointer">
                                <input id="admin" type="radio" class="form-radio h-5 w-5 text-green-600 transition duration-150 ease-in-out" name="level" value="2" required @checked(old('level', $user->level) == 2)>
                                <span class="ml-3 block text-sm font-medium text-gray-700">Admin</span>
                            </label>
                            
                            
                            @else    
                            
                            @if (old('level', $user->level) == 0 || Auth::user()->level == 2)
                    ...
                    <div id="koordinator-select" class="hidden">
                        <label for="koordinator" class="block mb-2 text-xs font-medium text-black">Koordinator</label>
                        <select name="koordinator_id" id="koordinator" class="shadow-sm bg-gray-50 border border-gray-300 text-black text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5">
                            <option value="">Pilih Koordinator</option>
                            @foreach ($koordinators as $item)
                                <option value="{{ $item->id }}" @selected(old('koordinator_id', $user->koordinator_id) == $item->id)>{{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <script>
                        const koordinatorSelect = document.getElementById('koordinator-select');
                        const surveyor = document.getElementById('surveyor');
                        const koordinator = document.getElementById('koordinator_surveyor');

                        function toggleKoordinatorSelect() {
                            if (surveyor.checked) {
                                koordinatorSelect.classList.remove('hidden');
                            } else {
                                koordinatorSelect.classList.add('hidden');
                            }
                        }

                        surveyor.addEventListener('change', toggleKoordinatorSelect);
                        koordinator.addEventListener('change', toggleKoordinatorSelect);
                        
                        // Initial state
                        toggleKoordinatorSelect();
                    </script>
                </div>
            </div>
        </section>
